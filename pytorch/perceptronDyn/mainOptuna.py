...
def objective(trial):
    ...
    d_learn_rate = trial.suggest_loguniform("d_learn_rate", 1e-4, 1e-2)
    ...
    for epoch in range(constants.EPOCHS):
        ...
        for i, (images, _) in enumerate(route_loader):
            ...
            for step in range(constants.K):
                data_fake = generator(
                    create_noise(curr_batch_size, constants.NOISE_DIM)
                )
                data_real = images
            ...
            g_loss += train_generator(
                discriminator, g_optim, data_fake, eval_tensor, epoch
            )
