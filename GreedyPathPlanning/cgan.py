...
Original file is located at
    https://colab.research.google.com/drive/1c7N9BKmkxfVmPJE3K97i2XhHp_QD7kUh
"""

"""https://www.youtube.com/watch?v=MAMSh5kVoec&t=193s
...
from numpy import zeros, ones, array, array2string
...
def define_discriminator(in_shape=(6,100,1)):
	model = Sequential()
	
	model.add(Conv2D(128, (3,3), strides=(2,2), padding='same', input_shape=in_shape)) #3x50x128
	model.add(LeakyReLU(alpha=0.2))
	
	model.add(Conv2D(128, (3,3) , padding='same')) #3x50x128
	model.add(LeakyReLU(alpha=0.2))
	
	model.add(Flatten()) #shape of 19200
...
def define_generator(latent_dim):    #latent_dim is the dimension of the latent vector (e.g., 100)
	...
	n_nodes = 128 * 3 * 50  #19200 nodes
	model.add(Dense(n_nodes, input_dim=latent_dim)) #Dense layer so we can work with 1D latent vector
	model.add(LeakyReLU(alpha=0.2))
	model.add(Reshape((3, 50, 128)))  #8x8x128 dataset from the latent vector. 
	# upsample to 100x6
	model.add(Conv2DTranspose(128, (4,4), strides=(2,2), padding='same')) #6x100x128
	model.add(LeakyReLU(alpha=0.2))
	# 
	model.add(Conv2DTranspose(128, (4,4), padding='same')) #6x100x128
	model.add(LeakyReLU(alpha=0.2))
	# generate
	model.add(Conv2D(1, (8,8), activation='tanh', padding='same')) #6x100x1
...
# load dataset
  ...
  trainX = array(trainX)
  trainy = array(trainy)
  with open('RealSample.txt', 'w') as writefile:
    writefile.write(array2string(trainX))
  # # plot 25 images
  # for i in range(1):
  # 	plt.subplot(5, 5, 1 + i)
  # 	plt.axis('off')
  ...
  # X = (X - 4) / 4   #Generator uses tanh activation so rescale 
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=100, n_batch=128):
	bat_per_epo = 100 #int(dataset.shape[0] / n_batch)
	...
	g_model.save('trainedGenerator.h5')
...
dataset = dataset.reshape(-1, 6, 100, 1)
#print(dataset.shape)
# train model
...
def show_plot(examples, n):
  examples = examples.reshape(-1,6,100)
  for i in range(n * n):
    plt.subplot(n, n, 1 + i)
    plt.axis('off')
...
# load model
model = load_model('trainedGenerator.h5') #Model trained for 100 epochs
# generate images
