...
from keras import layers, Sequential, metrics, Model, utils, datasets, optimizers, losses
# from tensorflow.keras import layers # This works
...
class ConditionalGAN(Model):
    def __init__(self, discriminator: Sequential, generator: Sequential, latent_dim):
...
def interpolate_class(first_number, second_number):
    ...
    first_label = utils.to_categorical([first_number], num_classes)
    second_label = utils.to_categorical([second_number], num_classes)
...
converted_images = tf.image.resize(
    converted_images, (96, 96)).numpy().astype(np.uint8)
